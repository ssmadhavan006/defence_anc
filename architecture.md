...
│   ├── dashboard.py
│   ├── spectrogram.py
│   ├── backup_playback.py    (Phase 5.1) real-corpus 60s clip + real-time feed source
│   ├── backup_audio/          generated by backup_playback.py --generate, gitignored (*.wav)
│   ├── run_judged_demo.sh    (Phase 5.2) launches exactly one UI process (see its own header
│   │                          note on why dashboard/spectrogram/webdash can't run concurrently)
│   ├── ps26052-demo.service  (Phase 5.3a) systemd auto-restart unit template
...
- **2026-09-04 (Phase 5.1-5.3 — demo bulletproofing, dev-machine work only):** Built exactly the three requested sections. **5.1** `demo/backup_playback.py` — a 60s clip built from real corpus sources (ESC-50 engine, Zenodo gunshot, LibriSpeech speech) via the same `mix_signals()` the eval numbers use; `BackupAudioSource` feeds it into the pipeline's input ring buffer at real-time cadence. `live/pipeline.py`'s `--backup <path>` skips opening `sd.InputStream` (and the dual-mic reference stream) entirely, leaving the real output stream untouched so judges still hear actual hardware output; `demo/webdash/app.py` got the same flag. **5.2** `demo/run_judged_demo.sh` — first found a real architectural constraint (Rule 27): `demo/dashboard.py`/`spectrogram.py`/`webdash/app.py` each independently open their own exclusive `sd.InputStream`/`OutputStream`, so they cannot run as simultaneous separate processes against the same hardware the way the plan's literal wording implied; the script instead launches exactly one UI process (default `webdash`) after preflight + QR generation. A real bug was found and fixed during testing: `--stop` issued during the auto-restart wrapper's 2s cooldown gap found no live child process and silently no-op'd; fixed by tracking the wrapper's own PID in a separate supervisor-PID file, verified against a real running session. **5.3** RTF health check in `live/pipeline.py` (`_check_rtf_health()`, pure/unit-tested): sustained RTF>0.9 for a full 5.0s of real time triggers a one-way auto-bypass (default on — deliberately different from `dnsmos.auto_bypass`'s default-off, since RTF is an objective measurement, not a noisy subjective quality score); `scripts/preflight_check.py` (red/green, composes into the demo script, run for real on this dev machine — genuinely reports what's attached here, no fabricated hardware); `demo/ps26052-demo.service` as the systemd alternative to the script's own `--auto-restart` loop. Self-test suite: 22 PASS + 3 correct SKIP (was 20+3), zero regressions. **Not done, and deliberately out of this pass's scope:** 5.4 (deck) and 5.5 (video) were not requested. **Not done, Mode B:** cold-boot-to-demo timing, a real induced pipeline crash against the auto-restart wrapper, a real sustained-overload condition against the RTF health check, full dress rehearsal. Full detail: `till_now.md`, `progress.md`.
