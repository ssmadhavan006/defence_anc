...
│       │   └── gunshot/
│       ├── non_stationary/
│       │   ├── aircraft/            # corpus v2 (ESC-50 'airplane')
│       │   ├── crowd/               # RETIRED in corpus v2 -- on disk, not declared in CATEGORIES
│       │   ├── helicopter/
│       │   └── wind/                # corpus v2 (ESC-50 'wind')
│       └── stationary/
│           ├── engine/
│           └── vehicle/
├── demo/
...
- **2026-09-04 (Corpus v2 — `non_stationary` redefinition, `docs/corpus_redefinition_v2.md`):** The `non_stationary` category's `crowd` subtype was retired and replaced with `wind` (ESC-50 class 16) + `aircraft` (ESC-50 class 47 `airplane`), both extracted from the already-downloaded ESC-50 archive via the new `scripts/extract_esc50_subtype.py` (no new download, Rule 15). Two independent reasons, both established *before* the change and pre-registered in `docs/corpus_redefinition_v2.md` so the replacement classes were chosen on threat-model grounds rather than by score: (1) multi-talker babble is *speaker separation*, a different problem class from the speech *enhancement* PS26052 asks for; (2) the subtype was **ill-posed as constructed** — `scripts/generate_babble_noise.py` drew babble from the same `data/clean` pool as the target speech with no exclusion, and that pool holds only **2 unique LibriSpeech speakers**, so **39/40** crowd mixtures contained the target speaker's own voice inside the interferer (4/40 the literal same utterance). That also supersedes the earlier explanation for the anomalous Phase 3 T6 oracle-NLMS inversion (Rule 27): the "oracle" reference was partly the target signal. `stationary` and `impulsive` pairings were verified **byte-identical** to v1 and act as a control. v1 artefacts preserved (`data/manifest_v1_crowd.csv`, `results/v1_crowd/`). Also added `eval/make_compliance_report.py` so the compliance verdict is computed from `eval_raw.csv` by a command rather than hand-assembled as it had been (Rules 1/3), with per-subtype breakdown — category means are exactly what hid the `crowd` defect. **This change alters the evaluation, not the system**; the enhancement pipeline is bit-identical across it, and `docs/corpus_redefinition_v2.md` §6 states in binding terms how the v1→v2 numbers may and may not be described. Known remaining defect, deliberately left for a separate single-variable change: the clean pool uses 2 of the 40 available `dev-clean` speakers.
