...
## CURRENT STATUS
- Phase: **5 — COMPLETE** ✅ | Post-Phase-5 hardening COMPLETE ✅ | Latency engineering (P0-2/P0-3) COMPLETE on loopback ✅ | P1-1/P1-3/P1-4 COMPLETE (dev-verified; P1-3 confirmed non-viable on the Pi's Python 3.13) ✅
- Last updated: 2026-08-24 23:20:00
- What works right now: everything from the previous status entry, plus: **P0-5 spectrogram mechanically verified on the Pi** (46s live run, ANSI rendering correct, `b`/`q` responsive, 0 real dropouts, RTF 0.336–0.418 — panels rendered fully saturated rather than dark because the closed `snd-aloop` self-loop has no external signal to calibrate against, exactly the degenerate outcome flagged in advance; will read correctly once a physical mic is attached). **P1-4 data augmentation** (`data/augment.py`, RIR + clipping, no scipy dependency) built and Pi-verified. **P1-1 residual filter** (`live/residual_filter.py`, reference-free ALE) built, self-tests and a 60s Pi stress test both clean (0 dropouts); audio quality not yet validated, stays off by default. **P1-3 ONNX backend** (`models/deepfilternet/export_onnx.py`/`onnx_infer.py`) built, bit-exact vs PyTorch, ~42% faster on the dev machine — but confirmed via PyPI metadata to be **fundamentally incompatible with the Pi's Python 3.13** (`onnx`'s `ml_dtypes` dependency requires numpy≥2.1 there; deepfilternet requires numpy<2.0; no resolvable combination exists). Pi dependency chain fully repaired and hardened (`requirements.txt` now core-only and always installs; `requirements-optional.txt` holds everything else) after a real regression was introduced and caught (a disabled feature's import could take down the whole pipeline — fixed to lazy-import). `live/latency_test.py` gained `--backend pytorch|onnx` for comparing inference backends. `scripts/make_demo_clip.py` (new) generates an illustrative before/after clip from the project's own noise corpus; one was generated, processed through `run_inference.py`, and delivered to the user as real audio evidence — the first actual listened-to output this project has produced, as opposed to only measured metrics. `run_inference.py --input-dir` (fixed earlier this session) is now documented in README.md as a general-purpose "bring your own audio" capability, independent of the dataset/manifest.
- What's broken / blocked: nothing on the software side; `run_all_selftests.py` is 9/9 on the dev machine and 7 PASS + 2 correct SKIP on the Pi (ONNX tests skip because their dependency is deliberately not installed there). **Still blocked on physical hardware (P0-1)** — every audio measurement to date, including e2e latency and the spectrogram content, runs through `snd-aloop` (a virtual ALSA loopback), never a physical microphone or headset.
- Waiting on user for: a USB microphone + headphones/headset (P0-1) — needed for P0-4 (real-mic stress test) and to make P0-5's spectrogram show real content. Also: the presentation deck and backup demo video (P1-6) — not hardware-blocked, not started, and per the plan the single largest remaining gap that isn't waiting on a mic.
- Next immediate action: acquire physical audio hardware (P0-1); once available, re-run `detect`, hardcode device indices, re-run the 10-minute stress gate with real acoustic noise (P0-4), and re-check the spectrogram now that there's a real signal source. In parallel, start P1-6 (deck + backup video) — it does not need to wait for the mic. See `summary/02_NEXT_STEPS_PLAN.md` for the full remaining plan.
